...
void APP_PXPInit()
{
    ...
    /* PS configure. */
    psBufferConfig.pixelFormat = kPXP_PsPixelFormatRGB565; //kPXP_PsPixelFormatUYVY1P422;//kPXP_PsPixelFormatRGB565;//kPXP_PsPixelFormatRGB888;//kPXP_PsPixelFormatRGB565;
...
#else
    if(p_isLiveView)
        APP_PXPAlpha(1,0, 0, LCD_WIDTH-1, LCD_HEIGHT-1, 0xb0U, 1);
    else
...
static void PXP_Task(void* param)
{
    ...
    while (1)
    {
        ...
        if (ret == pdTRUE)
        {
            ...
            #if CAMERA_ROTATE_FLAG
                 if (pQMsg->msg.pxp.type == 1) {
                    APP_SetPxpRotate(pQMsg->msg.pxp.in_buffer, (uint32_t)rotateBuff);
                    util_GRAY16ToRGB888((uint16_t *)rotateBuff, APP_CAMERA_WIDTH,APP_CAMERA_HEIGHT,(uint8_t *)pQMsg->msg.pxp.out_buffer);
                 } else if (pQMsg->msg.pxp.type == 2) {
                    APP_SetPxpRotate(pQMsg->msg.pxp.in_buffer, (uint32_t)rotateBuff);
                    util_Raw16ToDepth12((uint16_t *)rotateBuff, APP_CAMERA_WIDTH,APP_CAMERA_HEIGHT,(uint16_t *)pQMsg->msg.pxp.out_buffer);
                 }
            #else
                 if (pQMsg->msg.pxp.type == 1) {
                    util_GRAY16ToRGB888((uint16_t *)pQMsg->msg.pxp.in_buffer, APP_CAMERA_WIDTH,APP_CAMERA_HEIGHT,(uint8_t *)pQMsg->msg.pxp.out_buffer);
                 } else if (pQMsg->msg.pxp.type == 2) {
                    util_Raw16ToDepth12((uint16_t *)pQMsg->msg.pxp.in_buffer, APP_CAMERA_WIDTH,APP_CAMERA_HEIGHT,(uint16_t *)pQMsg->msg.pxp.out_buffer);
                 }
            #endif
                pQMsg = &FResMsg;
            ...
            #if CAMERA_ROTATE_FLAG
                APP_SetPxpRotate(pQMsg->msg.pxp.in_buffer,(uint32_t)rotateBuff);
                util_GRAY16ToRGB565((uint16_t *)rotateBuff, APP_CAMERA_WIDTH,APP_CAMERA_HEIGHT,tempBuff);
                APP_PXPStartCamera2Display((uint32_t)tempBuff, *pQMsg->msg.pxp.info, 1, pQMsg->msg.pxp.out_buffer);
            #else
                util_GRAY16ToRGB565((uint16_t *)pQMsg->msg.pxp.in_buffer, APP_CAMERA_WIDTH,APP_CAMERA_HEIGHT,tempBuff);
                APP_PXPStartCamera2Display((uint32_t)tempBuff, *pQMsg->msg.pxp.info, 1, pQMsg->msg.pxp.out_buffer);
            #endif
                pQMsg = &DResMsg;
                Camera_SendQMsg((void*)&pQMsg);
