...
void APP_PXPInit()
{
    ...
    asBlendConfig.alpha = 0xFU;          /* Don't care. */
    asBlendConfig.invertAlpha = false; /* Don't care. */
    asBlendConfig.alphaMode = kPXP_AlphaRop; //kPXP_AlphaRop;
...
void APP_PXPStartCamera2Display(uint32_t cameraBuffer, QUIInfoMsg infoMsg, uint8_t p_isLiveView, uint32_t outBuffer)
{
    ...
    APP_PXPOut(LCD_WIDTH, LCD_HEIGHT,
                LCD_WIDTH - APP_CAMERA_WIDTH/scale_factor, LCD_HEIGHT - APP_CAMERA_HEIGHT/scale_factor,
                LCD_WIDTH - 1, LCD_HEIGHT - 1,
                APP_CAMERA_WIDTH, APP_CAMERA_HEIGHT, APP_CAMERA_WIDTH/scale_factor, APP_CAMERA_HEIGHT/scale_factor,
                kPXP_OutputPixelFormatRGB565, LCD_HEIGHT * 2);
    APP_PXPStart((uint32_t)cameraBuffer, outBuffer, 90);
#else
    APP_PXPOut(LCD_WIDTH, LCD_HEIGHT,
                LCD_WIDTH - APP_CAMERA_WIDTH/scale_factor, LCD_HEIGHT - APP_CAMERA_HEIGHT/scale_factor,
                LCD_WIDTH - 1, LCD_HEIGHT - 1,
    ...
    APP_PXPStart((uint32_t)cameraBuffer, outBuffer, 0);
#endif
}
...
void APP_PXPStartCamera2DetBuf(uint32_t cameraBuffer, uint32_t detBuffer)
{
    ...
    APP_PXPOut(APP_CAMERA_WIDTH, APP_CAMERA_HEIGHT,
               0, 0, APP_CAMERA_WIDTH - 1, APP_CAMERA_HEIGHT - 1,
               APP_CAMERA_WIDTH, APP_CAMERA_HEIGHT, APP_CAMERA_WIDTH, APP_CAMERA_HEIGHT,
               kPXP_OutputPixelFormatRGB888P, APP_CAMERA_WIDTH * 3);
