...
static uint8_t timer_rec_no_face_ID             = TIMER_DET_NO_FACE;
...
{
    s_Timer_Reg_No_Face   =
        xTimerCreate("RegTimerNoFace", pdMS_TO_TICKS(MS_MANUAL_REG_NO_FACE), pdFALSE, (void *)&timer_reg_no_face_ID, pxCallbackFunction);
    if (s_Timer_Reg_No_Face == NULL )
    {
        return -1;
    }
    s_Timer_Reg_Has_Face  =
        xTimerCreate("RegTimerHasFace", pdMS_TO_TICKS(MS_MANUAL_REG_HAS_FACE), pdFALSE, (void *)&timer_reg_has_face_ID, pxCallbackFunction);
    if (s_Timer_Reg_Has_Face == NULL )
    {
        return -1;
    }
    s_Timer_Dereg_No_Face   =
        xTimerCreate("DeregTimerNoFace", pdMS_TO_TICKS(MS_MANUAL_REG_NO_FACE), pdFALSE, (void *)&timer_dereg_no_face_ID, pxCallbackFunction);
    if (s_Timer_Dereg_No_Face == NULL )
    {
        return -1;
    }
    s_Timer_Dereg_Has_Face  =
        xTimerCreate("DeRegTimerHasFace", pdMS_TO_TICKS(MS_MANUAL_REG_HAS_FACE), pdFALSE, (void *)&timer_dereg_has_face_ID, pxCallbackFunction);
    if (s_Timer_Dereg_Has_Face == NULL )
    {
        return -1;
    }
    s_Timer_Lock  =
        xTimerCreate("TimeLockOasis", pdMS_TO_TICKS(MS_SYSTEM_LOCKED), pdFALSE, (void *)&timer_lock_ID, pxCallbackFunction);
    if (s_Timer_Lock == NULL )
    {
        return -1;
    }
