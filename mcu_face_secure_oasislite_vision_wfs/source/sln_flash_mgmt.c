...
static sln_encrypt_ctx_t s_flashMgmtEncCtx = {
    .key =  {0x2c, 0x7d, 0x13, 0x18, 0x26, 0xb0, 0xd0, 0xaa,
        0xab, 0xf7, 0x16, 0x88, 0x09, 0xcf, 0x4f, 0x3e},

    .iv = {0xef, 0xf0, 0xf9, 0xec, 0xfc, 0xf1, 0xf4, 0xf9,
        0xf8, 0xf9, 0xfa, 0x02, 0xfc, 0xfd, 0xfe, 0xff},
...
static int32_t get_file_data(file_meta_t *meta, uint8_t *data)
{
    ...
    if (meta->useEncryption)
    {
        ...
        ret = SLN_Decrypt_AES_CBC_PKCS7(&s_flashMgmtEncCtx, (uint8_t*)meta->fileDataAddr, meta->dataCryptLen, data, &(meta->dataPlainLen));
...
static int32_t set_file_data(file_meta_t *meta, uint8_t *flashFile, uint8_t *data)
{
    ...
    if (meta->useEncryption)
    {
        ...
        ret = SLN_Encrypt_AES_CBC_PKCS7(&s_flashMgmtEncCtx, data, meta->dataPlainLen, flashFile + sizeof(sln_file_header_t), meta->dataCryptLen);
...
static int32_t init_ram_crc_mirror(uint32_t flashEntryIdx)
{
    ...
    if (NULL != s_fileLock)
    {
        ...
        if (pdTRUE == xSemaphoreTake(s_fileLock, portMAX_DELAY))
        {
            ...
            // Calculate CRC
            ret = calc_crc_32(&meta, (uint8_t *)(meta.fileDataAddr), flashHdr->sizeBytes);
...
static int32_t garbage_collector(uint32_t flashEntryIdx)
{
    ...
    if (mapIdx > GC_THRESHOLD)
    {
        ...
        fileSize = currHdr.sizeBytes + sizeof(sln_file_header_t);
...
int32_t SLN_FLASH_MGMT_Save(const char *name, uint8_t *data, uint32_t len)
{
    ...
    if (NULL != s_fileLock)
    {
        if (pdTRUE == xSemaphoreTake(s_fileLock, portMAX_DELAY))
        {
            ...
            newHdr->valid = 1;
            newHdr->clean = 1;
            newHdr->reserved = 0xFFFF;
            newHdr->sizeBytes = meta->useEncryption ? meta->dataCryptLen : meta->dataPlainLen;
            ...
            ret = calc_crc_32(meta, flashFile + sizeof(sln_file_header_t), newHdr->sizeBytes);
...
int32_t SLN_FLASH_MGMT_Update(const char *name, uint8_t *data, uint32_t *len)
{
    ...
    if (NULL != s_fileLock)
    {
        if (pdTRUE == xSemaphoreTake(s_fileLock, portMAX_DELAY))
        {
            ...
            // Run crc on file data
            ret = calc_crc_32(meta, flashFile + sizeof(sln_file_header_t), currHdr->sizeBytes);
...
int32_t SLN_FLASH_MGMT_Read(const char *name, uint8_t *data, uint32_t *len)
{
    ...
    if (NULL != s_fileLock)
    {
        if (pdTRUE == xSemaphoreTake(s_fileLock, portMAX_DELAY))
        {
            ...
            ret = set_file_size_info(meta, flashHdr->sizeBytes);
            ...
            msgDec = (uint8_t *)pvPortMalloc(flashHdr->sizeBytes);
            ...
            // Copy data into temporary buffer; decryption will recove true plain text length
