...
{
    return (uint32_t)((plainLen / AES_BLOCK_SIZE) * AES_BLOCK_SIZE + ((plainLen % AES_BLOCK_SIZE) ? AES_BLOCK_SIZE : 0));
}
 ...
 */
int32_t SLN_Encrypt_AES_CBC_PKCS7(sln_encrypt_ctx_t *ctx, const uint8_t *in, size_t inSize, uint8_t *out, size_t outSize);
 ...
 * @returns 0 in case of success or a negative value in case of error
 */
int32_t SLN_Decrypt_AES_CBC_PKCS7(sln_encrypt_ctx_t *ctx, const uint8_t *in, size_t inSize, uint8_t *out, size_t *outSize);
