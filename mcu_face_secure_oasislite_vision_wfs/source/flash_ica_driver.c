 ...
 * Definitions
 ******************************************************************************/
#define UART_DEBUG(x) configPRINTF(x)
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
...
int32_t FICA_write_image_info(int32_t imgtype, uint32_t imgfmt, uint32_t len)
{
    ...
    if (SLN_FLASH_NO_ERROR == status)
    {
        ...
        UART_DEBUG(("[FICA] Flash ICA record written successfully\r\n"));
    }
...
bool is_FICA_initialized(void)
{
    ...
    {
        UART_DEBUG(("[FICA] Flash ICA already initialized\r\n"));

        ret = true;
    }
    else
    {
        UART_DEBUG(("[FICA] Flash ICA needs initialization\r\n"));
    }
...
int32_t FICA_initialize(void)
{
    ...
    UART_DEBUG(("[FICA] Checking Image Config Area (ICA) initialization\r\n"));
    ...
    if (!isInitialized)
    {
        UART_DEBUG(("[FICA] Flash ICA initialization started\r\n"));
        ...
        if (kStatus_Success == status)
        {
            ...
            s_fica.header.communication = FICA_COMM_AIS_OTA_BIT;
    ...
    UART_DEBUG(("[FICA] Flash ICA initialization complete\r\n"));
...
int32_t FICA_verify_ext_flash(void)
{
    ...
    UART_DEBUG(("[FICA] RT HYPERFLASH Check...\r\n"));

    // Read flash vendor ID
    if (FICA_read_hyper_flash_id(pid) != SLN_FLASH_NO_ERROR)
    {
        UART_DEBUG(("[FICA] ERROR: Cannot find Flash device!\r\n"));

        return (SLN_FLASH_ERROR);
    }
    else
    {
        UART_DEBUG(("[FICA] Found Flash device!\r\n"));
    }
    ...
    // Read flash vendor ID
    if (flexspi_nor_qspi_flash_id(FLEXSPI, id) != SLN_FLASH_NO_ERROR)
    {
        UART_DEBUG(("[FICA] ERROR: Cannot find Flash device!\r\n"));
