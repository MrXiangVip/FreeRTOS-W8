...
NonCachedNonInit __attribute__((aligned(8))) uint8_t usb_serialHandleBuffer_2[SERIAL_MANAGER_HANDLE_SIZE];
NonCached  serial_handle_t usb_serialHandle[USB_DEVICE_CONFIG_CDC_ACM] = { &usb_serialHandleBuffer_1, &usb_serialHandleBuffer_2 };
...
__attribute__((aligned(4))) USB_LINK_NONCACHE_DATA  static uint8_t s_currRecvBuf[USB_DEVICE_CONFIG_CDC_ACM][DATA_BUFF_SIZE];
        ...
        case kUSB_DeviceCdcEventSendResponse:
        {
            if ((1 == serialUsbCdc->attach))
            {
                if ((epCbParam->length != 0) &&
                    (!(epCbParam->length % serialUsbCdc->bulkInEndpointMaxPacketSize)))
                {
        ...
        case kUSB_DeviceCdcEventSerialStateNotif:
            ...
            if ((serialUsbCdc->startTransactions) && (serialUsbCdc->tx.waiting4Prime))
            {
                ...
                if (kStatus_USB_Success != USB_DeviceCdcAcmSend(serialUsbCdc->cdcAcmHandle,
                                                                serialUsbCdc->bulkInEndpoint,
                                                                serialUsbCdc->tx.buffer,
...
usb_status_t USB_DeviceCdcVcomSetConfigure(class_handle_t handle, uint8_t configure)
{
    if (USB_COMPOSITE_CONFIGURE_INDEX == configure)
    {
        ...
        USB_DeviceCdcAcmRecv(s_deviceComposite->cdcVcom[0].cdcAcmHandle, s_deviceComposite->cdcVcom[0].bulkOutEndpoint,
                s_deviceComposite->cdcVcom[0].currRecvBuf, s_deviceComposite->cdcVcom[0].bulkOutEndpointMaxPacketSize);

        /*endpoint information for cdc 2*/
        s_deviceComposite->cdcVcom[1].attach = 1;
        s_deviceComposite->cdcVcom[1].currRecvBuf = s_currRecvBuf[1];
        s_deviceComposite->cdcVcom[1].interruptEndpoint = USB_CDC_VCOM_CIC_INTERRUPT_IN_ENDPOINT_2;
        s_deviceComposite->cdcVcom[1].interruptEndpointMaxPacketSize = g_cdcVcomCicEndpoints_2[0].maxPacketSize;

        s_deviceComposite->cdcVcom[1].bulkInEndpoint = USB_CDC_VCOM_DIC_BULK_IN_ENDPOINT_2;
        s_deviceComposite->cdcVcom[1].bulkInEndpointMaxPacketSize = g_cdcVcomDicEndpoints_2[0].maxPacketSize;

        s_deviceComposite->cdcVcom[1].bulkOutEndpoint = USB_CDC_VCOM_DIC_BULK_OUT_ENDPOINT_2;
        s_deviceComposite->cdcVcom[1].bulkOutEndpointMaxPacketSize = g_cdcVcomDicEndpoints_2[1].maxPacketSize;

        /* Schedule buffer for receive */
        USB_DeviceCdcAcmRecv(s_deviceComposite->cdcVcom[1].cdcAcmHandle, s_deviceComposite->cdcVcom[1].bulkOutEndpoint,
                s_deviceComposite->cdcVcom[1].currRecvBuf, s_deviceComposite->cdcVcom[1].bulkOutEndpointMaxPacketSize);
    }
...
usb_status_t USB_DeviceCdcVcomInit(usb_device_composite_struct_t *deviceComposite)
{
    ...
    for (uint8_t i = 0; i < USB_DEVICE_CONFIG_CDC_ACM; i++)
    {
        ...
        serialConfig.cdcHandleAdress = &(s_deviceComposite->cdcVcom[i]);
