...
#if RTFFI_USB_DISK
    {
        USB_DeviceMscCallback,     (class_handle_t)NULL, &g_UsbDeviceMscDiskConfig,
    },
#endif
#if RTFFI_USB_VIDEO
    {
        USB_DeviceVideoCallback,   (class_handle_t)NULL, &g_UsbDeviceVideoCameraConfig[0],
    },
#endif
};
...
usb_device_class_config_list_struct_t g_UsbDeviceCompositeConfigList = {
    g_CompositeClassConfig, USB_DeviceCallback, USB_DEVICE_CONFIG_TOTAL,
};
...
usb_status_t USB_DeviceCallback(usb_device_handle handle, uint32_t event, void *param)
{
    ...
    (defined(USB_DEVICE_CONFIG_LPCIP3511HS) && (USB_DEVICE_CONFIG_LPCIP3511HS > 0U))
            ...
            {
                g_composite.g_UsbDeviceVideoCamera.currentMaxPacketSize = HS_STREAM_IN_PACKET_SIZE * (1 + HS_STREAM_IN_PACKET_ADDITIONAL_TRANSACTION);
            }
            else
            {
...
void USB_DeviceApplicationInit(void)
{
    ...
    else
    {
        ...
        g_composite.cdcVcom[0].cdcAcmHandle = g_UsbDeviceCompositeConfigList.config[i++].classHandle;
        g_composite.cdcVcom[0].deviceHandle = g_composite.deviceHandle;
        g_composite.cdcVcom[1].cdcAcmHandle = g_UsbDeviceCompositeConfigList.config[i++].classHandle;
        g_composite.cdcVcom[1].deviceHandle = g_composite.deviceHandle;
        USB_DeviceCdcVcomInit(&g_composite);

#if  RTFFI_USB_DISK
        g_composite.mscDisk.mscHandle = g_UsbDeviceCompositeConfigList.config[i++].classHandle;
        g_composite.mscDisk.deviceHandle = g_composite.deviceHandle;
        USB_DeviceMscDiskInit(&g_composite);
#endif

#if  RTFFI_USB_VIDEO
        g_composite.g_UsbDeviceVideoCamera.videoHandle = g_UsbDeviceCompositeConfigList.config[i++].classHandle;
    ...
    USB_DeviceIsrEnable(&g_composite);
