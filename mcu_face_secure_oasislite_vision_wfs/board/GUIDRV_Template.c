...
*/
  ...
  #elif ( LCD_MIRROR_X && !LCD_MIRROR_Y && !LCD_SWAP_X
    #define LOG2PHYS_Y(x, y) yY)
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  LCD_PIXELINDEX PixelIndex;
  int x;
  int rect_size = (x1-x0+1) * (y1-y0+1);

  GUI_PORT_API *pContext = &((DRIVER_CONTEXT_TEMPLATE *)pDevice->u.pContext)->portApi;
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    for (; y0 <= y1; y0++) {
      ...
      pContext->pfWrite8_A0(0x2a);
      pContext->pfWrite8_A1((x0 >> 8) & 0xff);
      pContext->pfWrite8_A1(x0 & 0xff);
      pContext->pfWrite8_A1((x1 >> 8) & 0xff);
      pContext->pfWrite8_A1(x1 & 0xff);

      pContext->pfWrite8_A0(0x2b);
      pContext->pfWrite8_A1((y0 >> 8) & 0xff);
      pContext->pfWrite8_A1(y0 & 0xff);
      pContext->pfWrite8_A1((y1 >> 8) & 0xff);
      pContext->pfWrite8_A1(y1 & 0xff);

      pContext->pfWrite8_A0(0x2c);
      for(int i = 0; i < rect_size; i++)
      {
          ColorVetor[i] = PixelIndex;
      }
      U16 *p = ColorVetor;
      int major_loop = rect_size /30/1024;
      for(int i = 0; i < major_loop; i++) {
          pContext->pfWriteM16_A1(p, 30*1024);
          p += 30*1024;
      }
      int minor_loop = rect_size % (30 *1024);
      pContext->pfWriteM16_A1(p, minor_loop);
#if 0
    for (; y0 <= y1; y0++) {
      for (x = x0; x <= x1; x++) {
...
static void _DrawBitmap(GUI_DEVICE * pDevice, int x0, int y0,
  ...
  case 1:
      ...
      pContext->pfWrite8_A1((x0 >> 8) & 0xff);
      pContext->pfWrite8_A1(x0 & 0xff);
      pContext->pfWrite8_A1(((x0 + xSize) >> 8) & 0xff);
      pContext->pfWrite8_A1((x0 + xSize) & 0xff);

      pContext->pfWrite8_A0(0x2b);
      pContext->pfWrite8_A1((y0 >> 8) & 0xff);
      pContext->pfWrite8_A1(y0 & 0xff);
      pContext->pfWrite8_A1(((y0 + ySize) >> 8) & 0xff);
      pContext->pfWrite8_A1((y0 + ySize) & 0xff);
      ...
      for(int i = 0; i < ySize; i++)
      {
          ...
          for(int j = 0; j < xSize; j++)
          {
              if(*p & (0x80 >> Diff))
              {
                  ColorVetor[i*xSize + j] = *pTrans;
              }
              else
              {
                  ColorVetor[i*xSize + j] = 0x00;
              }
              if(++Diff == 8)
              {
                  Diff = 0;
      ...
      pContext->pfWriteM16_A1(ColorVetor, xSize*ySize);
...
#if 1
  {
      ...
      pContext->pfWrite8_A1((x0 >> 8) & 0xff);
      pContext->pfWrite8_A1(x0 & 0xff);
      pContext->pfWrite8_A1(((x0 + xSize - 1) >> 8) & 0xff);
      pContext->pfWrite8_A1((x0 + xSize - 1) & 0xff);

      pContext->pfWrite8_A0(0x2b);
      pContext->pfWrite8_A1((y0 >> 8) & 0xff);
      pContext->pfWrite8_A1(y0 & 0xff);
      pContext->pfWrite8_A1(((y0 + ySize - 1) >> 8) & 0xff);
      pContext->pfWrite8_A1((y0 + ySize - 1) & 0xff);

      pContext->pfWrite8_A0(0x2c);
      int major_loop = xSize * ySize /30/1024;
      for(int i = 0; i < major_loop; i++) {
          pContext->pfWriteM16_A1((U16 *)pData, 30*1024);
          pData += 30*1024;
      }
      int minor_loop = (xSize*ySize) % (30 *1024);
      pContext->pfWriteM16_A1((U16 *)pData, minor_loop);
  }
#endif
#if 0
    for (i = 0; i < ySize; i++) {
...
static I32 _GetDevProp(GUI_DEVICE * pDevice, int Index) {
  ...
  case LCD_DEVCAP_NUMCOLORS:
    return 1 << pContext->BitsPerPixel;
...
void GUIDRV_Template_SetFunc(GUI_DEVICE *pDevice, GUI_PORT_API *PortAPI)
{
    _InitOnce(pDevice);
    GUI_PORT_API *pContext;
    pContext = &((DRIVER_CONTEXT_TEMPLATE *)pDevice->u.pContext)->portApi;
    pContext->pfWrite8_A0 = PortAPI->pfWrite8_A0;
    pContext->pfWrite8_A1 = PortAPI->pfWrite8_A1;
    pContext->pfWriteM8_A1 = PortAPI->pfWriteM8_A1;
    pContext->pfRead8_A1 = PortAPI->pfRead8_A1;
    pContext->pfReadM8_A1 = PortAPI->pfReadM8_A1;

    pContext->pfWrite16_A0 = PortAPI->pfWrite16_A0;
    pContext->pfWrite16_A1 = PortAPI->pfWrite16_A1;
    pContext->pfWriteM16_A1 = PortAPI->pfWriteM16_A1;
    pContext->pfRead16_A1 = PortAPI->pfRead16_A1;
    pContext->pfReadM16_A1 = PortAPI->pfReadM16_A1;
}
