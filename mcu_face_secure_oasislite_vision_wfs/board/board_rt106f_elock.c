...
#endif
lpi2c_rtos_handle_t g_i2c1_handle,g_i2c4_handle;
/*******************************************************************************
 * Code
 ******************************************************************************/
...
void BOARD_LPI2C_Init(LPI2C_Type *base, uint32_t clkSrc_Hz)
{
    ...
    if ((g_i2c1_handle.base == NULL) && (base == LPI2C1))
    {
        LPI2C_MasterGetDefaultConfig(&lpi2cConfig);
        LPI2C_RTOS_Init(&g_i2c1_handle, base, &lpi2cConfig, clkSrc_Hz );
        NVIC_SetPriority(LPI2C1_IRQn, configMAX_SYSCALL_INTERRUPT_PRIORITY - 1);
    }
    else if ((g_i2c4_handle.base == NULL) && (base == LPI2C4))
    {
        LPI2C_MasterGetDefaultConfig(&lpi2cConfig);
        LPI2C_RTOS_Init(&g_i2c4_handle, base, &lpi2cConfig, clkSrc_Hz );
        NVIC_SetPriority(LPI2C4_IRQn, configMAX_SYSCALL_INTERRUPT_PRIORITY - 1);
    }
}
                          ...
                          uint8_t txBuffSize)
{
    ...
    if (2U == subAddressSize)
    {
        /* Byte swap. */
        /* This swap is caused by a bug in SKD fixed in 2.7 . Should eliminate it once we merge to 2.7
         BUG_SDK*/
        transfer.subaddress = ((subAddress >> 8U) & 0xFF) | ((subAddress & 0xFFU) << 8U);
    }else
    {
    ...
    {
        if (base == LPI2C1)
            reVal = LPI2C_RTOS_Transfer(&g_i2c1_handle, &transfer);
        else if(base == LPI2C4)
            reVal = LPI2C_RTOS_Transfer(&g_i2c4_handle, &transfer);
    }
...
#if CAMERA_DIFF_I2C_BUS && !CAMERA_SUPPORT_3D
...
status_t BOARD_3DCamera_I2C_Send(
    uint8_t deviceAddress, uint32_t subAddress, uint8_t subAddressSize, const uint8_t *txBuff, uint8_t txBuffSize)
{
    return BOARD_LPI2C_Send(BOARD_3DCAMERA_I2C_BASEADDR, deviceAddress, subAddress, subAddressSize, (uint8_t *)txBuff,
                            txBuffSize);
}
...
status_t BOARD_3DCamera_I2C_Receive(
    uint8_t deviceAddress, uint32_t subAddress, uint8_t subAddressSize, uint8_t *rxBuff, uint8_t rxBuffSize)
{
    return BOARD_LPI2C_Receive(BOARD_3DCAMERA_I2C_BASEADDR, deviceAddress, subAddress, subAddressSize, rxBuff,
                               rxBuffSize);
}
...
status_t BOARD_Camera_IR_I2C_SendSCCB(
    uint8_t deviceAddress, uint32_t subAddress, uint8_t subAddressSize, const uint8_t *txBuff, uint8_t txBuffSize)
{
    return BOARD_LPI2C_SendSCCB(BOARD_CAMERA_IR_I2C_BASEADDR, deviceAddress, subAddress, subAddressSize, (uint8_t *)txBuff,
                                txBuffSize);
}
...
status_t BOARD_Camera_IR_I2C_ReceiveSCCB(
    uint8_t deviceAddress, uint32_t subAddress, uint8_t subAddressSize, uint8_t *rxBuff, uint8_t rxBuffSize)
{
    return BOARD_LPI2C_ReceiveSCCB(BOARD_CAMERA_IR_I2C_BASEADDR, deviceAddress, subAddress, subAddressSize, rxBuff,
                                   rxBuffSize);
}
...
void BOARD_ConfigMPU(void)
{
    ...
    /* Region 10 setting, set last 2MB of SDRAM can't be accessed by cache, glocal variables which are not expected to be
