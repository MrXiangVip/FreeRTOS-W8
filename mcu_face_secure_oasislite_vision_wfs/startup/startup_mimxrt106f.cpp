//*****************************************************************************
// MIMXRT106F startup code for use with MCUXpresso IDE
//
// Version : 190818
//*****************************************************************************
//
...
// FlexRAM sections defined by Managed Linker
...
#define TEMP_STACK_TOP (__base_SRAM_OC_NON_CACHEABLE + 0x8000) // Temporary stack pointer to end of first block of SRAM OC (always there)
...
void (* const g_pfnVectors[])(void) = {
    // Core Level - CM7
    TEMP_STACK_TOP,                       // The initial stack pointer
...
__attribute__((section(".after_vectors.reset"))) void ResetISR(void)
{
    ...
    // Reconfigure FlexRAM to match linker settings
    map_flexram();

    // Set stack pointer to applicaiton stack location
    volatile unsigned int vStackTop = (unsigned int)&_vStackTop;
    __asm volatile("ldr sp, %0" : : "m"(vStackTop));
