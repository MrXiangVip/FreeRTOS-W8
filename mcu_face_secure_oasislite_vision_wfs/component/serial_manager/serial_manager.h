...
typedef enum _serial_manager_status
{
    ...
    kStatus_SerialManager_NotConnected =
        MAKE_STATUS(kStatusGroup_SERIALMANAGER, 7), /*!< The ring buffer is overflowed */
...
extern "C"
{
     ...
     *  @endcode
     *
     * @param serialHandle Pointer to point to a memory space of size #SERIAL_MANAGER_HANDLE_SIZE allocated by the
     * caller.
     ...
     * copied from ring buffer to up layer buffer. The copied length is the minimum of the ring buffer and up layer
     * length. After the data is copied, the actual data length is passed by the parameter length. And There can only
     * one buffer for receiving for the reading handle at the same time.
     *
     * @param readHandle The serial manager module handle pointer.
     * @param buffer Start address of the data to store the received data.
     * @param length The length of the data to be received.
     ...
     *
     * The function cancels unfinished send transmission. When the transfer is canceled, the module notifies the upper
     * layer through a TX callback function and passes the status parameter @ref kStatus_SerialManager_Canceled.
     *
     ...
     *
     * The function cancels unfinished receive transmission. When the transfer is canceled, the module notifies the
     * upper layer through a RX callback function and passes the status parameter @ref kStatus_SerialManager_Canceled.
     *
