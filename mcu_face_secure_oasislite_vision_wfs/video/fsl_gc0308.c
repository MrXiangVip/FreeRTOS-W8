...
//	GC0308_set_orientation(handle, true, false);					// option to flip the image (horizontal and / or vertical flip)
//	GC0308_set_white_balance(handle, kwhite_balance_auto); 			// option to tune the white balance
//	GC0308_set_brightness(handle, -1);								// option to adjust the brightness
 ...
 */
static status_t GC0308_CropImage(camera_device_handle_t *handle, uint16_t window_width, uint16_t window_height, uint16_t origin_x, uint16_t origin_y )
{
...
static status_t GC0308_RegInit(camera_device_handle_t *handle, const camera_config_t *config)
{
	...
	case kVIDEO_ResolutionVGA:
		/* GC0308 has a native resolution of 640x480. This is already handled in GC0308_SensorInitialSetting function */
		break;
	case kVIDEO_ResolutionQVGA:
		status = GC0308_CropImage(handle, 320U, 240U, (uint16_t)((GC0308_SENSOR_WIDTH - 320U)/2), (uint16_t)((GC0308_SENSOR_HEIGHT - 240U)/2));
		break;
	case FSL_VIDEO_RESOLUTION(480, 272):
		status = GC0308_CropImage(handle, 480U, 272U, (uint16_t)((GC0308_SENSOR_WIDTH - 480U)/2), (uint16_t)((GC0308_SENSOR_HEIGHT - 272U)/2));
		break;
	case kVIDEO_ResolutionQQVGA:
		status = GC0308_CropImage(handle, 160U, 120U, (uint16_t)((GC0308_SENSOR_WIDTH - 160U)/2), (uint16_t)((GC0308_SENSOR_HEIGHT - 120U)/2));
		break;
	}
...
status_t GC0308_Init(camera_device_handle_t *handle, const camera_config_t *config)
{
	...
	if ((kVIDEO_PixelFormatRGB565 != config->pixelFormat) &&
			(kVIDEO_PixelFormatYUYV != config->pixelFormat) &&
			(kVIDEO_PixelFormatYVYU != config->pixelFormat) &&
			(kVIDEO_PixelFormatUYVY != config->pixelFormat) &&
	...
	if (GC0308_CHIP_ID != chip_id) {return kStatus_Fail;}
