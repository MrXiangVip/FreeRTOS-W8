...
#define MT9M114_Write(handle, reg, size, value)                                                    \
    VIDEO_I2C_WriteReg(((mt9m114_resource_t *)(handle->resource))->i2cAddr, kVIDEO_RegAddr16Bit, reg, (video_reg_width_t)size, value, \
                       ((mt9m114_resource_t *)(handle->resource))->i2cSendFunc)
#define MT9M114_Read(handle, reg, size, value)                                                    \
    VIDEO_I2C_ReadReg(((mt9m114_resource_t *)(handle->resource))->i2cAddr, kVIDEO_RegAddr16Bit, reg, (video_reg_width_t)size, value, \
                      ((mt9m114_resource_t *)(handle->resource))->i2cReceiveFunc)
#define MT9M114_Modify(handle, reg, size, clrMask, value)                                                    \
    VIDEO_I2C_ModifyReg(((mt9m114_resource_t *)(handle->resource))->i2cAddr, kVIDEO_RegAddr16Bit, reg, (video_reg_width_t)size, clrMask, value, \
                        ((mt9m114_resource_t *)(handle->resource))->i2cReceiveFunc,                          \
                        ((mt9m114_resource_t *)(handle->resource))->i2cSendFunc)
...
status_t MT9M114_Init(camera_device_handle_t *handle, const camera_config_t *config)
{
    ...
    if ((kVIDEO_Resolution720P != config->resolution) && (FSL_VIDEO_RESOLUTION(480, 272) != config->resolution) && (kVIDEO_ResolutionQVGA != config->resolution)
        && (kVIDEO_ResolutionVGA != config->resolution))
    {
        return kStatus_InvalidArgument;
    }
