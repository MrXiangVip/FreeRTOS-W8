...
static shell_status_t FFI_CLI_ListCommand(shell_handle_t shellContextHandle, int32_t argc, char **argv)
{
    ...
    if( argc > 2 ){
        ...
        } else if (strncmp(argv[1], "fd", 2) == 0) {
            testFeatureDownload(argv[2]);
        } else if (strncmp("cj", argv[1], 2) == 0) {
            SHELL_Printf(shellContextHandle, "test cjson\r\n");
            cJSON* root = cJSON_Parse(argv[2]);
        }
    }else if (argc == 2){
        if (strlen(argv[1]) == 1) {
            ...
            SHELL_Printf(shellContextHandle, "input command: list %s\r\n", argv[1]);
            if (strcmp("config", argv[1]) == 0) {
                print_project_config();
            } else if (strcmp("fd", argv[1]) == 0) {
                SHELL_Printf(shellContextHandle, "testFeatureDownload default\r\n");
                ...
                testFeatureDownload(featureJson);
