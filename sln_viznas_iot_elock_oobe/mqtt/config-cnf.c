...
static char g_fileBuffer[1024] = {0};
...
char *trim_string(char *string,int mode)
{
  ...
  if ((mode & 1) != 0) { // 去除左边空白字符
    for (;*left != '\0'; left++) {
      if (0 == isspace((unsigned char)*left)) {
        break;
      }
    }
  }
  if ((mode & 2) != 0) { // 去除右边空白字符
    ...
    for (;right >= left; right--) {
      if (0 == isspace((unsigned char)*right)) {
        *(right+1) = '\0';
        break;
      } 
    }
...
Config *cnf_read_config(const char *filename, char comment, char separator)
{
  ...
  fclose(fp);
    ...
    error = f_open(&g_fileObject, _T(filename), (FA_READ | FA_CREATE_NEW | FA_OPEN_APPEND));
    if (error)
    {
       if (error == FR_EXIST)
       {
	...
	char *s, *e, *pLine, sLine[MAX_VALUE];	  /* 保存一行数据到字符串 */
	char section[MAX_VALUE] = {'\0'}, key[MAX_VALUE], value[MAX_VALUE]; /* 缓存section,key,value */
	//while (NULL != f_read(&g_fileObject, sLine, MAX_VALUE, &num)) {
	while (NULL != f_gets(sLine, MAX_VALUE, &g_fileObject)) {
	  pLine = trim_string(sLine, TRIM_SPACE); /* 去掉一行两边的空白字符 */
	  if (*pLine == '\0' || *pLine == comment) {
		continue; /* 空行或注释行跳过 */
	  }
...
bool cnf_write_file(Config *cnf, const char *filename, const char *header)
{
  ...
  fclose(fp);
	...
	/*error = f_lseek(&g_fileObject, 0);
	if( error )
	{
		LOGD("Move file point failed: %d.\r\n", error);
	}*/

	if (!str_empty(header)) { /* 文件注释不为空,则写注释到文件 */
		memset(g_fileBuffer, 0, sizeof(g_fileBuffer));
		snprintf(g_fileBuffer, sizeof(g_fileBuffer), "%c %s\n\n", cnf->comment, header);
		LOGD("%s g_fileBuffer1 is %s\n", __FUNCTION__, g_fileBuffer);
		error = f_write(&g_fileObject, g_fileBuffer, strlen(g_fileBuffer), &num);
		if (error || (strlen(g_fileBuffer) != num))
		{
	...
	while (NULL != p) {
		memset(g_fileBuffer, 0, sizeof(g_fileBuffer));
		snprintf(g_fileBuffer, sizeof(g_fileBuffer), "[%s]\n", p->section);
		LOGD("%s g_fileBuffer2 is %s\n", __FUNCTION__, g_fileBuffer);
		error = f_write(&g_fileObject, g_fileBuffer, strlen(g_fileBuffer), &num);
		if (error || (strlen(g_fileBuffer) != num))
		{
			FATFS_PRINTF(("Write file failed: %d, write number: %d.\r\n", error, num));
		...
		while (NULL != q) {
			memset(g_fileBuffer, 0, sizeof(g_fileBuffer));
			snprintf(g_fileBuffer, sizeof(g_fileBuffer), "%s %c %s\n", q->key, cnf->separator, q->value);

			LOGD("%s g_fileBuffer3 is %s length is %d\n", __FUNCTION__, g_fileBuffer, strlen(g_fileBuffer));
#if	1
			error = f_write(&g_fileObject, g_fileBuffer, strlen(g_fileBuffer), &num);
			if (error || (strlen(g_fileBuffer) != num))
			{
				LOGD("Write file failed: %d, write number: %d.\r\n", error, num);
			}
#endif
