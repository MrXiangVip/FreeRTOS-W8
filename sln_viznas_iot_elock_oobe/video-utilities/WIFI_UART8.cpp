...
//    LOGD("\r\n%sSuccess to reset WiFi module\r\n", logTag);
    ...
    result = run_at_cmd("AT+ATE0", 1, 1200);
    ...
    result = quickConnectMQTT("f3eb0aee1eb7", "7CDFA102AB68", "0000000000000000", "10.0.14.61",
    ...
    do {
        ...
        if(count % 5 == 0) {
            if (mqtt_init_done == 1) {
                ...
                sprintf(pub_msg,
                        "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mac\\\":\\\"%s\\\"\\,\\\"wifi_rssi\\\":%s\\,\\\"battery\\\":%d\\,\\\"index\\\":%d\\,\\\"version\\\":\\\"%s\\\"}",
                        "0", "7CDFA101B1E6", "-55", 50, g_heartbeat_index++,
                ...
                int ret = quickPublishMQTTWithPriority(pub_topic, "{\\\"msgId\\\":\\\"0\\\"\\,\\\"mac\\\":\\\"7CDFA101B1E6\\\"\\,\\\"index\\\":2\\}", 1);
            }
        }
        count++;
